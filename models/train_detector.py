...
detector = ObjectDetector(classes=train.ind_to_classes, num_gpus=conf.num_gpus,
                          mode=mode, use_resnet=conf.use_resnet)
detector.cuda()
...
def train_batch(b):
    ...
    if not conf.gt_box:
        ...
        loss = class_loss
    # RPN loss
    if not conf.use_proposals and not conf.gt_box:
        ...
        res = pd.Series([rpn_class_loss.data, rpn_box_loss.data,
                         class_loss.data, box_loss.data, loss.data],
                        ['rpn_class_loss', 'rpn_box_loss', 'class_loss', 'box_loss', 'total'])
    elif not conf.gt_box:
        res = pd.Series([class_loss.data, box_loss.data, loss.data],
                        ['class_loss', 'box_loss', 'total'])
    else:
        res = pd.Series([class_loss.data, loss.data],
                        ['class_loss', 'total'])
