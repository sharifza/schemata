...
def filter_dets(boxes, obj_scores, obj_classes, rel_inds, pred_scores, asm_num):
    """
    Filters detections....
    ...
    for asm in range(1, asm_num + 1):
        ...
        assert obj_scores[obj_ind].size(0) == num_box

        assert obj_classes[obj_ind].size() == obj_scores[obj_ind].size()
        num_rel = rel_inds.size(0)
        assert rel_inds.size(1) == 2
        assert pred_scores[rel_ind].size(0) == num_rel

        obj_scores0 = obj_scores[obj_ind].data[rel_inds[:,0]]
        obj_scores1 = obj_scores[obj_ind].data[rel_inds[:,1]]

        pred_scores_max, pred_classes_argmax = pred_scores[rel_ind].data[:,1:].max(1)
        pred_classes_argmax = pred_classes_argmax + 1

        rel_scores_argmaxed = pred_scores_max * obj_scores0 * obj_scores1
        # rel_scores_argmaxed = torch.log(pred_scores_max) + torch.log(obj_scores0) + torch.log(obj_scores1)
        rel_scores_vs, rel_scores_idx = torch.sort(rel_scores_argmaxed.view(-1), dim=0, descending=True)

        rels.append(rel_inds[rel_scores_idx].cpu().numpy())
        pred_scores_sorted.append(pred_scores[rel_ind][rel_scores_idx].data.cpu().numpy())
        obj_scores_np.append(obj_scores[obj_ind].data.cpu().numpy())
        objs_np.append(obj_classes[obj_ind].data.cpu().numpy())
