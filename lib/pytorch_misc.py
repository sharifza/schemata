...
def print_para(model):
    ...
    return '\n {:.1f}M total parameters \n {:.1f}M total trainable parameters \n ----- \n \n{}\n'.format(
...
def apply_po_torch(self, img, img_size, gt_boxes, po_ratio=0.3,
                   max_box_size=0.5, min_olcn_size=0.1, max_ocln_size=1.0):
    """
    Make the image partially-observable by randomly occlude the bounding boxes in the input image
    :param img: input image
    :param img_size: image size
    :param gt_boxes: Ground truth boxes
    :param po_ratio: Partial observability ratio (The percentage of occluded boxes)
    :param max_box_size: Maximum size of a box allowed to be occluded (a ratio w.r.t image size)
    :param min_olcn_size: Minimum size of occlusion box (a ratio w.r.t box size)
    :param max_ocln_size: Maximum size of occlusion box (a ratio w.r.t box size)
    """
    # -- Determine the image dimensions and its area
    h, w, scale_factor = img_size
    image_area = w * h

    # -- Calculate boxes' area and discard those which are too big
    boxes_area = torch.prod(gt_boxes[:, 2:] - gt_boxes[:, :2], axis=1)
    boxes_filtered_idx = torch.where(boxes_area <= image_area * max_box_size)[0]
    boxes_filtered_len = boxes_filtered_idx.size(0)

    # -- Randomly select out of the remaining boxes
    choose_count = int(boxes_filtered_len * po_ratio)
    rnd_perm = torch.randperm(boxes_filtered_idx.size(0))
    boxes_final_idx = boxes_filtered_idx[rnd_perm[:choose_count]]
    boxes_final = gt_boxes[boxes_final_idx]

    # -- Calculate boxes' size
    boxes_size = boxes_final[:, 2:] - boxes_final[:, :2]

    # -- Determine occlusion-box position and size (randomly)
    rnd_size = torch.rand(choose_count, 2).cuda()
    rnd_pos = torch.rand(choose_count, 2).cuda()
    ocln_size = torch.max(boxes_size * min_olcn_size,
                          boxes_size * max_ocln_size * rnd_size)
    ocln_pos = rnd_pos * (boxes_size - ocln_size)

    # -- Apply the box offset, and convert from x,y,w,h to x1,y1,x2,y2
    # -- Implicit conversion from float to int
    olcn_boxes = torch.zeros_like(boxes_final, dtype=torch.int, device='cuda:0')
    olcn_boxes[:, :2] = ocln_pos + boxes_final[:, :2]
    olcn_boxes[:, 2:] = ocln_pos + boxes_final[:, :2] + ocln_size

    # -- Apply the occlusions
    # FIXME a parallel function for this?
    for x1, y1, x2, y2 in olcn_boxes:
        # -- Apply the occlusion
        img[:, y1:y2, x1:x2] = 0
