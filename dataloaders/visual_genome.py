...
from config import ModelConfig, VG_IMAGES, VG_DEPTH_IMAGES, IM_DATA_FN, VG_SGG_FN, VG_SGG_DICT_FN, BOX_SCALE, IM_SCALE, PROPOSAL_FN
from dataloaders.image_transforms import SquarePad, Grayscale, Brightness, Sharpness, Contrast, \
    RandomOrder, Hue, random_crop
from collections import defaultdict
from pycocotools.coco import COCO
# FIXME this module is loaded once in the train_rels.py or eval_rels.py
# i guess we have to pass the parameters from those file to here
# conf = ModelConfig()
...
class VG(Dataset):
    def __init__(self, mode, roidb_file=VG_SGG_FN, dict_file=VG_SGG_DICT_FN,
                 ...
                 use_proposals=False, izs_split=None):
        """
        ...
        self.split_mask, self.gt_boxes, self.gt_classes, self.relationships = load_graphs(
            ...
            filter_non_overlap=self.filter_non_overlap and self.is_train, izs_split=izs_split
        )

        # -- Load RGB images' filenames --
        self.filenames = load_image_filenames(image_file,VG_IMAGES)
...
def load_image_filenames(image_file, image_dir=VG_IMAGES):
    ...
    for i, img in enumerate(im_data):
        # -- (CHANGED) directly check img['image_id'] instead of the basename
        if img['image_id'] in corrupted_ids:
            continue
        # -- (CHANGED) Add file extension here
...
def load_graphs(graphs_file, mode='train', num_im=-1, num_val_im=0, filter_empty_rels=True,
                filter_non_overlap=False, visual_share=1.0, all_share=1.0, izs_split=None):
    """
    ...
    visual_share_num = int((image_index.size - num_val_im) * visual_share)
    end_ind = num_val_im + int((image_index.size - num_val_im) * all_share)
    if num_im > -1:
    ...
    if num_val_im > 0:
        if mode == 'val':
            ...
            image_index = image_index[num_val_im:end_ind]
            if izs_split:
                keep_idx = np.setdiff1d(np.arange(len(image_index)), izs_split)
                image_index = image_index[keep_idx]
...
class VGDataLoader(torch.utils.data.DataLoader):
    ...
    @classmethod
    def splits(cls, train_data, val_data, batch_size=3, num_workers=1, num_gpus=3, mode='det', **kwargs):
        assert mode in ('det', 'rel')
        train_load = cls(
            dataset=train_data,
