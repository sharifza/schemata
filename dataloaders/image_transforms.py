...
class SquarePad(object):
    # -- (CHANGED) Changed function signature
    def __init__(self, single_channel=False):
        # -- (ADDED) Set the image mean (fill the padding with this number)
        if not single_channel:
            self.mean = (int(0.485 * 256), int(0.456 * 256), int(0.406 * 256))
        else:
            # -- TODO change this number with the real mean
            self.mean = 32767

    def __call__(self, img):
        w, h = img.size
        # -- (CHANGED) replace the filling value
